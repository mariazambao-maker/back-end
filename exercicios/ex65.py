from random import randint
print('Vamos jogar par ou Impar')
cont = 0
while True:
    palpite = int(input('Diga un valor'))
    escolha = str(input('Par ou Impar?')).lower()
    pc = randint(1,10)
    resultado = palpite + pc
    if resultado % 2 == 0 and escolha == 'par' or resultado % 2 != 0 and escolha == 'impar':
        print(f'Você jogou (palpite) e o computador jogou (pc) total deu (resultado) dau (escolha)')
        cont += 1
        print('Você VENCEU!')
    else:
        escolhapc = 'par' if resultado % 2 == 0 else'impar'
        print(f'Voce jogou {palpite} e o computador jogou {pc} total deu {resultado} dau {escolhapc}')
        break
    print('Vamos jogar novamente...')
print('Game Over! Você venceu {cont} vezes')
